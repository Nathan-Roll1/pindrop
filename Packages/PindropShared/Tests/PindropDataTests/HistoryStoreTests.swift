...
struct HistoryStoreTests {
    ...
    @Test func diskBackedMigrationFromV3PreservesExistingTranscriptions() throws {
        ...
        do {
            let legacySchema = Schema(versionedSchema: TranscriptionRecordSchemaV3.self)
    ...
    @Test func diskBackedMigrationFromV4LeavesExistingTranscriptionsUnfiled() throws {
        ...
        do {
            let legacySchema = Schema(versionedSchema: TranscriptionRecordSchemaV4.self)
            let legacyContainer = try ModelContainer(
                for: legacySchema,
                configurations: [ModelConfiguration(schema: legacySchema, url: storeURL)]
            )
    ...
    @Test func exportToJSON() throws {
        ...
        let payload = try HistoryExportEncoder.encode(
            records: records,
            format: .json,
        ...
        #expect(payload.suggestedFilename == "transcription_history.json")
        ...
        let exportData = try decoder.decode(ExportData.self, from: payload.data)
        ...
        #expect(exportData.exportDate.contains("2023-11-14") || exportData.exportDate.contains("2023"))
    }
    ...
    @Test func exportToCSV() throws {
        ...
        #expect(payload.suggestedFilename == "transcription_history.csv")
        let csvContent = String(decoding: payload.data, as: UTF8.self)
        let csvLines = csvContent.components(separatedBy: "\n").filter { !$0.isEmpty }

        #expect(csvLines.count == 3)
        #expect(csvLines[0].starts(with: "ID,Timestamp,Duration,Model,Original Text,Enhanced Text,Was Enhanced"))
        #expect(csvLines[1].contains("\"Text with \"\"quotes\"\" and, commas\""))
        #expect(csvLines[1].contains("10.00"))
        #expect(csvLines[1].contains("base"))
        #expect(csvLines[2].contains("\"First transcription\""))
        #expect(csvLines[2].contains("5.00"))
        #expect(csvLines[2].contains("tiny"))
    }

    @Test func exportEmptyRecords() throws {
        do {
            _ = try HistoryExportEncoder.encode(
                records: [],
                format: .json,
