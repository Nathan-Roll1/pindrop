...
struct DictionaryStoreTests {
    private func makeStore() throws -> DictionaryStore {
        ...
        return DictionaryStore(modelContext: ModelContext(modelContainer))
    }
    ...
    @Test func persistentDictionaryRelaunchesAndExports() throws {
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("pindrop-dictionary-relaunch-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directoryURL) }
        let storeURL = directoryURL.appendingPathComponent("dictionary.store")

        try seedPersistentDictionary(at: storeURL)

        let container = try PindropModelContainerFactory.makeContainer(at: storeURL)
        let reloadedStore = DictionaryStore(modelContext: ModelContext(container))
        let replacements = try reloadedStore.fetchAllReplacements()
        let vocabulary = try reloadedStore.fetchAllVocabularyWords()

        #expect(replacements.count == 1)
        #expect(replacements.first?.originals == ["pin drop"])
        #expect(replacements.first?.replacement == "Pindrop")
        #expect(vocabulary.map(\.word) == ["WhisperKit"])

        let exportData = try reloadedStore.exportToJSON()
        let export = try #require(
            JSONSerialization.jsonObject(with: exportData) as? [String: Any]
        )
        #expect(export["version"] as? Int == 1)
        #expect((export["replacements"] as? [[String: Any]])?.count == 1)
        #expect((export["vocabulary"] as? [[String: Any]])?.count == 1)
    }

    private func seedPersistentDictionary(at storeURL: URL) throws {
        let container = try PindropModelContainerFactory.makeContainer(at: storeURL)
        let store = DictionaryStore(modelContext: ModelContext(container))
        try store.add(
            WordReplacement(
                originals: ["pin drop"],
                replacement: "Pindrop",
        ...
        try store.add(VocabularyWord(word: "WhisperKit"))
    }
}
