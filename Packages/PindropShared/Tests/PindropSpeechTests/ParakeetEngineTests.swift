...
struct ParakeetEngineTests {
    ...
    private func makeDownloadBase(for modelName: String) throws -> (URL, URL) {
        let (locations, root) = try SpeechTestSupport.makeStorageLocations(label: "parakeet")
        let directory = ParakeetEngine.modelDirectory(
            forName: modelName,
    ...
    @Test func loadModelSetsStateToReady() async throws {
        ...
        do {
            let (downloadBase, root) = try makeDownloadBase(for: "parakeet-tdt-0.6b-v2")
            defer { try? FileManager.default.removeItem(at: root) }
            try await engine.loadModel(modelName: "parakeet-tdt-0.6b-v2", downloadBase: downloadBase)
        } catch {
        }
    ...
    @Test func v3ModelVersionSelection() async throws {
        ...
        Task {
            do {
                let (downloadBase, root) = try makeDownloadBase(for: "parakeet-tdt-0.6b-v3")
                defer { try? FileManager.default.removeItem(at: root) }
                try await engine.loadModel(modelName: "parakeet-tdt-0.6b-v3", downloadBase: downloadBase)
            } catch {
            }
        }
