...
private enum DiarizationIntegrationFixtureSupport {
...
private let diarizationIntegrationPrerequisitesAvailable: Bool = {
    ...
    let root = DiarizationIntegrationFixtureSupport.fixtureRoot
...
struct DiarizationQualityIntegrationTests {
    ...
    fileprivate static var fixtureRoot: URL {
        DiarizationIntegrationFixtureSupport.fixtureRoot
    }
    ...
    @Test func publicFixturesProduceExpectedSpeakerCounts() async throws {
        for fixture in try fixtures() {
            ...
            let modelsRoot = try SpeechTestSupport.makeStorageLocations(label: "diarization-int").locations.fluidAudioModelsRoot
            let diarizer = FluidSpeakerDiarizer(modelsDirectory: modelsRoot)
