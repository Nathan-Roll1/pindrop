...
struct AIEnhancementServiceTests {
    ...
    @Test func constructionDoesNotIssueRequest() {
...
// MARK: - Test Empty Text
    ...
    @Test func testBuildTranscriptionEnhancementInputRepairsNonfiniteConfidence() {
        let context = AIEnhancementService.ContextMetadata(
            hasClipboardText: false,
            hasClipboardImage: false,
            appContext: nil,
        ...
        #expect(payload.contains("<active_file_confidence>0.00</active_file_confidence>"))
        #expect(!(payload.contains("<workspace_confidence>")))
        #expect(!(payload.localizedCaseInsensitiveContains("nan")))
    }
