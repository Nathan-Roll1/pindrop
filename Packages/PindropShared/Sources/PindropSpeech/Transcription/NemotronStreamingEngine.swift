...
//  EOU signals.
//
//  modelsRoot must be the injected fluidAudioModelsRoot (…/FluidAudio/Models). Relative
//  load names resolve under that root; absolute paths are accepted as-is. Never
//  reconstruct Application Support.
//
...
public actor NemotronStreamingEngine: StreamingTranscriptionEngine {
    ...
    public init(
        chunkProfile: StreamingChunkProfile = .standard,
        modelsRoot: URL
    ) {
    ...
    private func resolveModelDirectory(for name: String) -> URL {
        ...
        return modelsRoot.appendingPathComponent(relativePath, isDirectory: true)
    }
