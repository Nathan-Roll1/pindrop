...
///
/// Model storage is always host-injected: callers pass the exact repository
/// directory (`fluidAudioModelsRoot` / `Repo.senseVoiceSmall.folderName`) as
/// `downloadBase`. The engine never reconstructs Application Support defaults.
@MainActor
public final class SenseVoiceEngine: TranscriptionEngine, CapabilityReporting {
    ...
    public nonisolated static let catalogPrecision: SenseVoiceEncoderPrecision = .int8
    ...
    public nonisolated static let maxWindowDurationSeconds = 100
    ...
    public nonisolated static let overlapDurationSeconds = 3
    ...
    public nonisolated static func requiredDownloadArtifacts(
        precision: SenseVoiceEncoderPrecision = catalogPrecision
    ) -> Set<String> {
        ModelNames.SenseVoice.requiredModels(precision: precision.rawValue)
    }

    /// Exact repository directory under an injected FluidAudio models root.
    public nonisolated static func modelDirectory(fluidAudioModelsRoot: URL) -> URL {
        fluidAudioModelsRoot.appendingPathComponent(
    ...
    /// Load SenseVoice from an injected repository directory, downloading into
    /// that directory's parent when artifacts are missing.
    ///
    /// - Parameters:
    ///   - name: Catalog model name (logged only; precision is always int8).
    ///   - downloadBase: Exact repository directory
    ///     `fluidAudioModelsRoot.appendingPathComponent(Repo.senseVoiceSmall.folderName)`.
    ///     Required. Never falls back to FluidAudio default Application Support paths.
    public func loadModel(name: String, downloadBase: URL?) async throws {
        ...
        do {
            guard let modelDirectory = downloadBase else {
                throw EngineError.initializationFailed(
                    "SenseVoice requires an injected model directory (downloadBase)."
                )
            }

            let precision = Self.catalogPrecision
            let loaded: SenseVoiceModels
            if SenseVoiceModels.modelsExist(at: modelDirectory, precision: precision) {
                loaded = try SenseVoiceModels.load(from: modelDirectory, precision: precision)
            } else {
                // SenseVoiceModels.download() always targets FluidAudio defaults.
                // Download into the parent of the injected repo directory instead,
                // so artifacts land at the exact host-supplied path.
                ...
                guard SenseVoiceModels.modelsExist(at: modelDirectory, precision: precision) else {
                    throw EngineError.downloadFailed(
                        "SenseVoice int8 artifacts incomplete after download at \(modelDirectory.path)"
                    )
                }
                loaded = try SenseVoiceModels.load(from: modelDirectory, precision: precision)
            }
            ...
            state = .ready
            Log.transcription.info(
                "SenseVoice ready name=\(name) precision=\(precision.rawValue) directory=\(modelDirectory.path)"
            )
        } catch let engineError as EngineError {
            self.error = engineError
            state = .error
    ...
    public nonisolated static func precision(forModelName name: String) -> SenseVoiceEncoderPrecision? {
    ...
    public nonisolated static func senseVoiceLanguageIndex(for language: AppLanguage) -> Int32 {
        switch language {
        case .automatic:
