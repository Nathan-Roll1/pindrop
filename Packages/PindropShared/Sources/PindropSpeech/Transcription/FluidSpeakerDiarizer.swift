...
//
//  Offline Community-1 diarizer. modelsDirectory must be the injected
//  fluidAudioModelsRoot (…/FluidAudio/Models). Never call
...
public final class FluidSpeakerDiarizer: PindropCore.SpeakerDiarizer {
    ...
    /// Injected FluidAudio models root (…/FluidAudio/Models).
    public let modelsDirectory: URL

    public private(set) var state: PindropCore.SpeakerDiarizerState = .unloaded
    public let mode: PindropCore.DiarizationMode = .offline

    private let modelsCache: FluidSpeakerDiarizerModelsCache

    public init(modelsDirectory: URL) {
        self.modelsDirectory = modelsDirectory
        self.modelsCache = FluidSpeakerDiarizerModelsCache(modelsDirectory: modelsDirectory)
    }
    ...
    public func loadModels() async throws {
        ...
        do {
            ...
            state = .ready
            Log.transcription.info("Speaker diarization model loaded (offline-community1) from \(self.modelsDirectory.path)")
    ...
    public func diarize(
        ...
        do {
            ...
            do {
                ...
                return PindropCore.DiarizationResult(segments: [], speakers: [], audioDuration: audioDuration)
            }
    ...
    private func mapResult(
    ...
    ) -> PindropCore.DiarizationResult {
        let sortedSegments = segments.sorted {
            $0.startTimeSeconds < $1.startTimeSeconds
        }

        var speakersByID: [String: PindropCore.Speaker] = [:]
        var speakersInOrder: [PindropCore.Speaker] = []
        var mappedSegments: [PindropCore.SpeakerSegment] = []
        ...
        for (index, segment) in sortedSegments.enumerated() {
            ...
            let speaker: PindropCore.Speaker
            if let existing = speakersByID[speakerID] {
                speaker = existing
            } else {
                let label = "Speaker \(speakersInOrder.count + 1)"
                let created = PindropCore.Speaker(id: speakerID, label: label, embedding: embedding)
...
private actor FluidSpeakerDiarizerModelsCache {
    ...
    func loadModels() async throws {
        ...
        let loaded = try await OfflineDiarizerModels.load(from: modelsDirectory)
        if models == nil {
            models = loaded
        }
    }
