...
/// On-device batch transcription via `SFSpeechRecognizer`.
///
/// Available on the package floors (macOS 14 / iOS 17). No downloaded model root:
/// authorization is the only load-time work; the recognizer is created per
/// transcription from `TranscriptionOptions.language`.
@MainActor
public final class AppleSpeechEngine: TranscriptionEngine, CapabilityReporting {
    ...
    public enum EngineError: Error, LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case .authorizationDenied:
                return "Speech recognition permission was denied. Enable it in system privacy settings."
