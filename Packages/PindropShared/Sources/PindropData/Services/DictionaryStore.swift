...
public struct LearnedReplacementChange: Equatable, Sendable {
    public let replacementID: UUID
    public let replacement: String
    public let learnedOriginal: String
    public let createdReplacement: Bool

    public init(
        replacementID: UUID,
        replacement: String,
        learnedOriginal: String,
        createdReplacement: Bool
    ) {
        self.replacementID = replacementID
        self.replacement = replacement
        self.learnedOriginal = learnedOriginal
        self.createdReplacement = createdReplacement
    }
}

@MainActor
public protocol LearnedReplacementPersisting: AnyObject {
...
@MainActor
@Observable
public final class DictionaryStore: LearnedReplacementPersisting {

    public enum DictionaryStoreError: Error, LocalizedError {
    ...
    public func fetchAllReplacements() throws -> [WordReplacement] {
    ...
    public func add(_ replacement: WordReplacement) throws {
    ...
    public func delete(_ replacement: WordReplacement) throws {
    ...
    public func reorder(_ replacements: [WordReplacement], from source: IndexSet, to destination: Int) throws {
        var reordered = replacements
        Self.moveElements(&reordered, fromOffsets: source, toOffset: destination)
    ...
    /// Foundation-only multi-index move with the same stable semantics as
    /// SwiftUI's `Array.move(fromOffsets:toOffset:)` (source order preserved,
    /// destination adjusted for removals that land before the insert point).
    private static func moveElements<Element>(
        ...
        let elementsToMove = sortedSources.map { array[$0] }

        var remaining: [Element] = []
        remaining.reserveCapacity(array.count - sortedSources.count)
        for (index, element) in array.enumerated() where !source.contains(index) {
            remaining.append(element)
        }

        let removedBeforeDestination = sortedSources.prefix(while: { $0 < destination }).count
    ...
    public func fetchAllVocabularyWords() throws -> [VocabularyWord] {
    ...
    public func add(_ word: VocabularyWord) throws {
    ...
    public func delete(_ word: VocabularyWord) throws {
    ...
    public func upsertLearnedReplacement(original: String, replacement: String) throws -> LearnedReplacementChange? {
    ...
    public func undoLearnedReplacement(_ change: LearnedReplacementChange) throws {
    ...
    /// - `.command` - word-boundary, case-insensitive spoken phrase to control sequence
    ...
    public func recordVocabularyHits(in text: String) throws {
    ...
    public func vocabularyBiasWords(limit: Int = VocabularyBiasPrompt.maxWordCount) throws -> [String] {
        let words = try fetchAllVocabularyWords()
    ...
    public static func wordBoundaryMatchCount(
        of pattern: String,
        in text: String,
    ...
    ///
    /// Uses lookarounds instead of classic `\b` word boundaries so punctuated
    /// tokens (e.g. `C++`) match correctly against whitespace and do not spuriously
    /// match prefixes of longer identifiers.
    public static func tokenBoundaryRegex(
        for pattern: String,
        caseInsensitive: Bool
    ...
    public func importFromJSON(_ data: Data, strategy: ImportStrategy) throws {
    ...
    public static func deduplicatedVocabularyWords(_ words: [String]) -> [String] {
        var seen = Set<String>()
        var result: [String] = []
