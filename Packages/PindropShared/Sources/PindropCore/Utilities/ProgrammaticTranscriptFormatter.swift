...
public struct ProgrammaticTranscriptFormatter: Sendable {
    ...
    public static let minimumSentenceCount = sentencesPerParagraph + 1

    /// Character floor after trimming; shorter text is left single-line.
    public static let minimumCharacterCount = 80

    /// Word floor; avoids pathological breaks on brief replies.
    public static let minimumWordCount = 12

    /// Sentences grouped into each paragraph before a blank line.
    public static let sentencesPerParagraph = 2
    ...
    public static let commonAbbreviations: Set<String> = [
    ...
    public static let trailingClosers = CharacterSet(charactersIn: "\"'”’)]}")
    ...
    public static func formatIfEnabled(_ text: String, enabled: Bool) -> String {
    ...
    public static func isProtectedPeriod(_ chars: [Character], at index: Int) -> Bool {
    ...
    public static func isSentenceBoundaryAfter(_ chars: [Character], at index: Int) -> Bool {
        if index + 1 >= chars.count {
            return true
        }
