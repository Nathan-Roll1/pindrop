...
2. **The load is bounded.** Wrap it in a 10 s timeout. On timeout, abandon the load, log once, and degrade to `.channel`. A load that takes longer than that is downloading, and it must not. The bound is measured, not assumed: on an M5 Pro a warm load of the balanced v2.1 bundle took 3.120 s and 3.145 s, and the first call, which also downloaded, took 62.998 s. An earlier 3 s bound sat under the warm load, so every capture timed out and silently degraded. 10 s is about three times the warm load, which absorbs Neural Engine contention and still refuses a download.
...
| Model load fails or times out | `LiveDiarizationEngine.load()` throws, or exceeds the 10 s bound (three times the measured 3.1 s warm load, see section 4.2) | Same as above, plus a logged `Log.transcription.error`. Banner text names the failure. | `The live speaker model could not be loaded. Recording continues, and the finished note still names everyone.` |
